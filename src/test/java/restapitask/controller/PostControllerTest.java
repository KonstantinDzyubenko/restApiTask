...
class PostControllerTest {
    ...
    @ParameterizedTest
    @MethodSource("getTestParameters")
    @WithMockUser(roles = {"POSTS_VIEWER", "POSTS_EDITOR"})
    ...
    private static Stream<Arguments> getTestParameters() {
        return Stream.of(
                arguments(get("/api/posts")),
                arguments(get("/api/posts/1")),
                arguments(get("/api/users/1/posts")),
                arguments(delete("/api/posts/1")),
                arguments(post("/api/posts").content("{}").contentType("application/json")),
                arguments(put("/api/posts/1").content("{}").contentType("application/json"))
        );
    }
}